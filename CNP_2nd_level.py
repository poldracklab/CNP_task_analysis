...
parser.add_argument('-contrast', '--contrast', dest='contrast',
                    help='contrast number', required=True)
parser.add_argument('--experiment', '--experiment', dest='experiment',
    ...
    for prep_pipeline in ['fmriprep-1.0.3', 'fslfeat_5.0.9']:
        ...
        for sample in range(2):
            ...
            for directory in [sampledir, outcopedir]:
                if not os.path.exists(directory):
                    os.mkdir(directory)
            copes = [os.path.join(x, 'stats', '%s%i.nii.gz' % ('cope', contrast))
                     for x in featdirs]
            varcopes = [os.path.join(x, 'stats', '%s%i.nii.gz' %
                                     ('varcope', contrast)) for x in featdirs]
            ...
            threscmd = 'fslmaths pstat1 -mul -1 -add 1 -thr %f -mas mask thresh_vox_fdr_pstat1' % (
                1 - thres)
            ...
            # cluster extent
            clustercmd = 'cluster -i zstat1 -c cope1 -t 3.2 -p 0.05 -d %s --volume=%s --othresh=thresh_cluster_fwe_zstat1 --connectivity=26 --mm' % (
                smoothn[0], smoothn[1])
