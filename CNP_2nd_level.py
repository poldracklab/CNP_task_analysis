...
for sub in os.listdir(featdir):
    ...
    if os.path.exists(subfeatdir):
        allfeatdirs.append(subfeatdir)

#################
## TEST-RETEST ##
#################
samplesize = 50
sampledfeatdirs = np.random.choice(len(allfeatdirs),samplesize*2,replace=False)
sampledfeatdirs = [sampledfeatdirs[:samplesize],sampledfeatdirs[samplesize:]]

for sample in range(2):
    print("STARTING ANALYSIS %i"%sample)
    featdirs = np.array(allfeatdirs)[sampledfeatdirs[sample]].tolist()
